...
class DrinksViewController: UITableViewController {
    ...
    private func setupUI() {
        tableView.register(DrinkCell.self, forCellReuseIdentifier: NSStringFromClass(DrinkCell.self))
    ...
    private func fetchCocktailsTypes() {
        Service().fetchCategories { [weak self] (result) in
            ...
            case .success(let categories):
                ...
                self?.navigationItem.rightBarButtonItem = .init(image: #imageLiteral(resourceName: "filter").withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(self?.filterButtonTaped))
            }
        }
    }
...
extension DrinksViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: NSStringFromClass(DrinkCell.self), for: indexPath) as? DrinkCell else { return UITableViewCell() }
